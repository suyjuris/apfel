...
struct Array_t {
    ...
    Array_t(std::initializer_list<T> lst):
...
template <typename T>
auto array_fwrite(FILE* file, Array_t<T> arr) {
    return fwrite(arr.data, sizeof(T), arr.size, file);
}
...
// Error handling
static Array_dyn<u8> global_err;

void error_print(char const* prefix = "Error") {
    s64 last = 0;
    for (s64 i = 0; i < global_err.size; ++i) {
        if (global_err[i] == '\n') {
            fprintf(stderr, "%s: ", prefix);
            array_fwrite(stderr, array_subarray(global_err, last, i));
            fputs("\n", stderr);
            last = i+1;
        }
    }
...
void error_errno(char const* message) {
    array_printf(&global_err, "%s\n%s\n", strerror(errno), message);
}

s64 array_load_from_file_try(Array_t<u8> path, Array_t<u8>* out_data) {
    auto error = [path](s64 code) {
        array_printf(&global_err, "while opening file '%s' for reading (%lld)\n", path.data, code);
        return code;
    };
    ...
    if (f == nullptr) {
        error_errno("while calling fopen()");
        return error(1025);
    }

    if (fseek(f, 0, SEEK_END) == -1) {
        error_errno("while calling fseek()");
        return error(1026);
    }

    s64 f_size = ftell(f);
    if (f_size == -1) {
        error_errno("while calling ftell()");
        return error(1027);
    }
    ...
    if (read < f_size) {
        if (ferror(f)) {
            error_errno("while calling fread()");
            return error(1029);
        } else {
            array_printf(&global_err, "Unexpected eof while reading file. (Concurrent modification?)");
            return error(1030);
        }
    }
    ...
    if (fclose(f)) {
        ...
        perror("Warning:");
        fprintf(stderr, "Warning: while closing file '%s'\n", path.data);
    }
...
Array_t<u8> array_load_from_file(Array_t<u8> path) {
    Array_t<u8> result;
    s64 code = array_load_from_file_try(path, &result);
    if (code) { error_print(); exit(code); }
    return result;
}
